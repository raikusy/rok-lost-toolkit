...
import { createFancyText } from "@/server/actions/fancy-text.action";
...
const CreateDialog = ({ editor }: { editor: Editor }) => {
  const form = useForm({
    defaultValues: {
      name: "",
      isPublic: false,
  ...
  const handleCreate = async (data: FancyTextInput) => {
    const content = JSON.parse(JSON.stringify(editor.getJSON()));
    startTransition(async () => {
      const created = await createFancyText({
        name: data.name,
        content: content,
        isPublic: !!data.isPublic,
      });
      router.push(PAGES.VIEW_TEMPLATE(created.id));
    });
  };

  return (
    <Dialog>
      <DialogTrigger className="btn-primary">Save Mail</DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Are you sure want to save this mail?</DialogTitle>
          <DialogDescription>
